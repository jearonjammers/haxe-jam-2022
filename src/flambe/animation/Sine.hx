 ...
 * @author Kipp Ashford
 */
class Sine implements Behavior {
	/** The end value. */
	...
	public function new(start:Float, end:Float, ?speed:Float = 1, ?cycles:Float = 0, ?offset:Float = 0) {
		...
		_initial = _count = HALF_PI + offset * (FMath.PI / speed); // Start at the start value plus the seconds to offset.
	...
	private static inline var HALF_PI:Float = {
		.5 * FMath.PI;
	};
